...
st.set_page_config(page_title="Data Collection & Visualization",
    page_icon="D:\Mon programme de MASTER_DIT\Master 1\Cours\Data Collection\Seances\Projet\Projet_examen_data_collection\icones\surveiller.png",layout="wide")
...
st.write("""Application de web scraping, nettoyage, stockage et visualisation des données.""")
...
#PAGE BOOKS
    ...
    #Filtre prix
    df_books_filtre = df_books_filtre[(df_books_filtre["Prix"]>= plage_prix_books[0])&(df_books_filtre["Prix"]<= plage_prix_books[1])]
    ...
    with col3:
        ...
        note_moyenne = (df_books_filtre["Note"].mean()if len(df_books_filtre) > 0 else 0)
...
#PAGE GAARAAS
    ...
    if boites:
        ...
        df_gaaraas_filtre = df_gaaraas_filtre[df_gaaraas_filtre["Boite_vitesse"].isin(boites)]
    ...
    with col1:
        ...
        st.metric(
            ...
            value=len(df_gaaraas_filtre)
        )

    with col2:

        prix_moyen_gaaraas = (df_gaaraas_filtre["Prix"].mean()
    ...
    with col3:
        ...
        kilometrage_moyen = (
            df_gaaraas_filtre["Kilometrage"].mean()
            if len(df_gaaraas_filtre) > 0
            else 0
        )
    ...
    if len(df_gaaraas_filtre) > 0:
        ...
        with col1:
            ...
            voitures_marque = (
                df_gaaraas_filtre[ "Marque"].value_counts().reset_index())
        ...
        prix_marque = (
            df_gaaraas_filtre.groupby("Marque")["Prix"].mean().reset_index().sort_values("Prix",ascending=False))
